...
(function() {
    ...
    // Инициализация темы (синхронизация переключателей, без изменения класса)
    function initTheme() {
        ...
        // Если пользователь авторизован, но тема в БД отличается – обновим
    ...
    // Глобальный переключатель темы (для использования из любого места)
            ...
            if (channelId) {
                ...
                fetch(`/api/channel/${channelId}/subscription-status`)
                    ...
                    .then(data => {
                        ...
                        // Обновляем оба счётчика подписчиков
                        ...
                        if (subscribersSpan) subscribersSpan.textContent = data.subscribers_count;
                        if (subscribersDisplaySpan) subscribersDisplaySpan.textContent = data.subscribers_count;
            ...
            fetch('/api/user/subscriptions-count')
                ...
                .then(data => {
                    if (data.count !== undefined) {
                        const subscriptionsStat = document.querySelector('.profile-stats .stat-item:last-child .stat-value');
                        if (subscriptionsStat) subscriptionsStat.textContent = data.count;
                        const sidebarSubscriptionsCount = document.querySelector('.sidebar-link[href*="subscriptions"] .badge');
                        if (sidebarSubscriptionsCount && data.count > 0) sidebarSubscriptionsCount.textContent = data.count;
                    }
                })
                .catch(console.error);
        }
    ...
    function updateSubscriptionsPage() {
        const subscriptionsPage = document.querySelector('.subscriptions-page');
        if (subscriptionsPage) location.reload();
    }
