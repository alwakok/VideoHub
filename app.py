...
class WatchRoom(db.Model):
    ...
    pending_play = db.Column(db.Boolean, default=False)  # Ожидание команды play
    pending_pause = db.Column(db.Boolean, default=False)  # Ожидание команды pause
    pending_time = db.Column(db.Float, default=None)  # Ожидаемое время
    pending_command_from = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=True)  # Кто отправил команду
    participants_confirmed = db.Column(db.Integer, default=0)  # Сколько подтвердило
    ...
    last_command = db.Column(db.String(20), nullable=True)  # 'play', 'pause', 'seek'
    last_command_value = db.Column(db.Float, nullable=True)  # время для seek
    last_command_time = db.Column(db.DateTime, nullable=True)  # когда команда была создана
...
def get_video_duration(file_path):
    """Получение реальной длительности видео"""
    try:
        # Пробуем использовать cv2 (opencv-python-headless)
        try:
            import cv2
            video = cv2.VideoCapture(file_path)
            if not video.isOpened():
                print(f"Cannot open video with cv2: {file_path}")
                return "0:00"
            ...
            if fps > 0 and frame_count > 0 and fps < 1000:  # проверка на валидность
        ...
        # Пробуем использовать ffprobe (если ffmpeg установлен)
        try:
            import subprocess
            import json
        ...
        # Если ничего не помогло, возвращаем "0:00"
...
def extract_video_thumbnail(video_path, thumbnail_path):
    """Извлечение превью из видео (первый кадр)"""
    try:
        # Пробуем использовать cv2 (opencv-python-headless)
        try:
            import cv2
            ...
            if not video.isOpened():
                print(f"Cannot open video for thumbnail: {video_path}")
                return False
            ...
            if success:
                ...
                print(f"Thumbnail created with cv2: {thumbnail_path}")
                return True
            else:
            ...
            print(f"CV2 thumbnail error: {e}")

        # Пробуем использовать ffmpeg
        try:
            import subprocess
            cmd = ['ffmpeg', '-i', video_path, '-ss', '00:00:01.000', '-vframes', '1', '-y', thumbnail_path]
            result = subprocess.run(cmd, capture_output=True, timeout=10)
            if result.returncode == 0 and os.path.exists(thumbnail_path) and os.path.getsize(thumbnail_path) > 0:
                print(f"Thumbnail created with ffmpeg: {thumbnail_path}")
                return True
        except Exception as e:
            print(f"FFmpeg thumbnail error: {e}")

        # Копируем дефолтную обложку
        ...
        if os.path.exists(default_thumbnail):
            ...
            print(f"Using default thumbnail: {thumbnail_path}")
            return True

        print(f"Could not create thumbnail for {video_path}")
        return False
    except Exception as e:
        ...
        if file and allowed_file(file.filename):
            ...
            if 'thumbnail' in request.files:
                ...
                if thumbnail_file and thumbnail_file.filename != '' and allowed_image_file(thumbnail_file.filename):
                    ...
                    print(f"User uploaded thumbnail: {thumbnail_filename}")

            # Если обложка не загружена, создаем из видео
            if not thumbnail_filename:
                ...
                if extract_video_thumbnail(video_path, thumbnail_path):
                    video.thumbnail = thumbnail_filename
                    print(f"Auto-generated thumbnail: {thumbnail_filename}")
                else:
            ...
            print(f"Video uploaded successfully: {video.id} - {video.title}")
...
def sync_watch_room(room_id):
    ...
    # Только создатель может менять состояние
...
def get_watch_room_state(room_id):
    ...
    video_info = None
    if room.video_id:
        video = db.session.get(Video, room.video_id)
        if video:
            video_info = {
                'id': video.id,
                'title': video.title,
    ...
    return jsonify({
        ...
        'room': {
            ...
            'last_sync': room.last_sync.isoformat() if room.last_sync else None,
            'video': video_info,
...
@app.route('/api/watch-room/<int:room_id>/command', methods=['POST'])
@login_required
def send_room_command(room_id):
    """Создатель отправляет команду"""
    room = WatchRoom.query.get_or_404(room_id)
    if room.created_by != current_user.id:
        return jsonify({'error': 'Только создатель может управлять'}), 403

    data = request.get_json()
    command = data.get('command')  # 'play', 'pause', 'seek'
    value = data.get('value')  # время для seek

    if command not in ['play', 'pause', 'seek']:
        return jsonify({'error': 'Неверная команда'}), 400

    room.last_command = command
    room.last_command_value = value if command == 'seek' else None
    room.last_command_time = datetime.utcnow()
    db.session.commit()

    return jsonify({'success': True, 'command': command, 'value': value})


@app.route('/api/watch-room/<int:room_id>/get_command', methods=['GET'])
@login_required
def get_room_command(room_id):
    """Участник получает последнюю команду"""
    room = WatchRoom.query.get_or_404(room_id)
    ...
    # Возвращаем команду, если она есть
    if room.last_command and room.last_command_time:
        # Важно: для seek возвращаем время, для play/pause — флаг
        response = {
            'has_command': True,
            'command': room.last_command,
            'value': room.last_command_value,
            'timestamp': room.last_command_time.isoformat()
        }
        # Опционально: очищаем команду после получения?
        # Лучше не очищать, чтобы несколько участников могли получить её.
        # Очистим после того, как все получат? Можно сделать по таймауту или после получения от всех.
        # Для простоты очистим после первого получения, но это не подходит для нескольких участников.
        # Альтернатива: хранить список получивших, но сложно.
        # Сделаем проще: команда действует 2 секунды, после чего удаляется (её можно получить несколько раз).
        # Удалим команду через 2 секунды в отдельном фоне, но проще на клиенте не применять повторно.
        # Оставим команду до следующей команды.
        return jsonify(response)

    return jsonify({'has_command': False})


@app.route('/api/watch-room/<int:room_id>/clear_command', methods=['POST'])
@login_required
def clear_room_command(room_id):
    """Участник подтверждает, что применил команду (опционально)"""
    room = WatchRoom.query.get_or_404(room_id)
    # Если участник подтвердил, можно удалить команду, но только если все подтвердили
    # Для простоты не будем использовать.
    return jsonify({'success': True})
...
def send_room_message(room_id):
    ...
    if not participant:
        return jsonify({'error': 'Вы не участник этой комнаты'}), 403
    message = RoomMessage(room_id=room_id, user_id=current_user.id, message=message_text)
...
def change_room_video(room_id):
    ...
    # Сбрасываем время и состояние воспроизведения
    ...
    # Получаем информацию о новом видео для ответа
    video_info = None
    if room.video_id:
...
def get_subscription_status(channel_id):
    ...
    # Добавляем количество подписок текущего пользователя
    user_subscriptions_count = Subscription.query.filter_by(
        subscriber_id=current_user.id).count() if current_user.is_authenticated else 0
...
# ========== НОВЫЕ МАРШРУТЫ ДЛЯ СИНХРОНИЗАЦИИ ==========

@app.route('/api/watch-room/<int:room_id>/request_play', methods=['POST'])
@login_required
def request_room_play(room_id):
    """Создатель запрашивает воспроизведение"""
    room = WatchRoom.query.get_or_404(room_id)
    ...
    data = request.get_json()
    current_time = data.get('current_time', room.current_video_time)

    # Устанавливаем ожидающую команду
    room.pending_play = True
    room.pending_pause = False
    room.pending_time = current_time
    room.pending_command_from = current_user.id
    room.participants_confirmed = 0  # Сбрасываем счётчик подтверждений

    db.session.commit()

    # Отправляем всем участникам (они будут опрашивать)
    return jsonify({'success': True, 'pending': True, 'current_time': current_time})


@app.route('/api/watch-room/<int:room_id>/request_pause', methods=['POST'])
@login_required
def request_room_pause(room_id):
    """Создатель запрашивает паузу"""
    room = WatchRoom.query.get_or_404(room_id)
    ...
    # Устанавливаем ожидающую команду
    room.pending_play = False
    room.pending_pause = True
    room.pending_command_from = current_user.id
    room.participants_confirmed = 0
...
@app.route('/api/watch-room/<int:room_id>/request_seek', methods=['POST'])
@login_required
def request_room_seek(room_id):
    """Создатель запрашивает перемотку"""
    room = WatchRoom.query.get_or_404(room_id)
    ...
    room.pending_time = seek_time
    room.pending_command_from = current_user.id
    room.participants_confirmed = 0

    db.session.commit()

    return jsonify({'success': True, 'pending': True, 'seek_time': seek_time})


@app.route('/api/watch-room/<int:room_id>/confirm_command', methods=['POST'])
@login_required
def confirm_room_command(room_id):
    """Участник подтверждает выполнение команды"""
    room = WatchRoom.query.get_or_404(room_id)
    participant = RoomParticipant.query.filter_by(room_id=room_id, user_id=current_user.id).first()
    ...
    room.participants_confirmed += 1

    # Получаем общее количество участников
    total_participants = RoomParticipant.query.filter_by(room_id=room_id).count()

    # Если все подтвердили (включая создателя)
    if room.participants_confirmed >= total_participants:
        # Применяем команду к состоянию комнаты
        if room.pending_play:
            room.is_playing = True
            room.current_video_time = room.pending_time
        elif room.pending_pause:
            room.is_playing = False

        if room.pending_time is not None:
            room.current_video_time = room.pending_time

        # Сбрасываем ожидающие команды
        room.pending_play = False
        room.pending_pause = False
        room.pending_time = None
        room.pending_command_from = None
        room.participants_confirmed = 0
        room.last_sync = datetime.utcnow()
    ...
    db.session.commit()
    return jsonify(
        {'success': True, 'applied': False, 'confirmed': room.participants_confirmed, 'total': total_participants})


@app.route('/api/watch-room/<int:room_id>/pending_command', methods=['GET'])
@login_required
def get_pending_command(room_id):
    """Получить ожидающую команду"""
    room = WatchRoom.query.get_or_404(room_id)
    ...
    response = {
        'has_pending': room.pending_play or room.pending_pause or room.pending_time is not None,
        'pending_play': room.pending_play,
        'pending_pause': room.pending_pause,
        'pending_time': room.pending_time,
        'command_from': room.pending_command_from,
        'total_participants': RoomParticipant.query.filter_by(room_id=room_id).count(),
        'confirmed': room.participants_confirmed
    }

    return jsonify(response)


@app.route('/api/user/subscriptions-count')
@login_required
def get_user_subscriptions_count():
    """Получить количество подписок текущего пользователя"""
    count = Subscription.query.filter_by(subscriber_id=current_user.id).count()
    return jsonify({'count': count})


@app.route('/api/user/<int:user_id>/subscriptions-count')
@login_required
def get_user_subscriptions_count_by_id(user_id):
    """Получить количество подписок пользователя по ID"""
    user = User.query.get_or_404(user_id)
    count = Subscription.query.filter_by(subscriber_id=user.id).count()
    return jsonify({'count': count})
...
def get_room_messages(room_id):
    """Получить новые сообщения в комнате (после last_id)"""
    room = WatchRoom.query.get_or_404(room_id)
    participant = RoomParticipant.query.filter_by(room_id=room_id, user_id=current_user.id).first()
    if not participant and room.created_by != current_user.id:
        return jsonify({'error': 'Вы не участник'}), 403
